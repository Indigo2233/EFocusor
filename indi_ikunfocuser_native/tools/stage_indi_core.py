#!/usr/bin/env python3
"""Stage the IKunFocuser driver into a clean indilib/indi checkout."""
...
set(ikunfocuser_SRC
    ikunfocuser.cpp
    ikunfocuser_protocol.cpp)

add_executable(indi_ikun_focuser ${ikunfocuser_SRC})
target_link_libraries(indi_ikun_focuser indidriver)
install(TARGETS indi_ikun_focuser RUNTIME DESTINATION bin)
"""

DRIVER_XML_BLOCK = """        <device label="IKun Focuser" manufacturer="IKun">
            <driver name="IKun Focuser">indi_ikun_focuser</driver>
            <version>1.0</version>
        </device>
"""
...
def main() -> None:
    ...
    for filename in ("ikunfocuser.cpp", "ikunfocuser.h", "ikunfocuser_protocol.cpp", "ikunfocuser_protocol.h"):
        shutil.copy2(source / filename, focuser_dir / filename)

    doc_dir = focuser_dir / "doc" / "ikunfocuser"
    doc_dir.mkdir(parents=True, exist_ok=True)
    shutil.copy2(source / "doc" / "index.md", doc_dir / "index.md")

    cmake_text = cmake_file.read_text(encoding="utf-8")
    if "add_executable(indi_ikun_focuser" not in cmake_text:
    ...
    if "<driver name=\"IKun Focuser\">" not in xml_text:
    ...
    print(f"IKunFocuser staged in {checkout}")
